...
- **Wheelbase-aware steering model:** DONE. Real future need identified during Milestone 4 physics discussion (Milestone 7 avoidance maneuvers need realistic turning geometry, not just direct heading-rotation) — deliberately deferred until collision (Milestone 4a) existed to validate against, then built as bicycle-model kinematics. See updated `vehicle/state.py` entry in section 3 above.
...
| 7–10 | Not started. See charter for full detail on each. Wheelbase-aware steering prerequisite is now done (see section 6 above) — Milestone 7 is unblocked. |

## 8. Immediate Next Task: Milestone 7 — Safety Experiments

Wheelbase-aware steering (the last prerequisite) is done. **Success criterion (charter section 29):** "Identical player inputs can be tested under different safety strategies."

**Context worth knowing before starting:** per `fleet-hardware.md`, the confirmed safety-intervention hypothesis — compatible with the stock drivetrain — is brake-impulse to load the front tires, followed by a timed steering input to rotate away from rigid barriers. Per-wheel torque vectoring is explicitly ruled out (no hardware path on stock Reflex 14 electronics). This milestone should assume brake+steer only, unless simulation testing conclusively proves that approach insufficient (see charter section 14 for the broader safety-system philosophy: invisible/minimally-intrusive, not "artificially safe").

Follow the same incremental, explain-then-implement working style used throughout this project so far (charter section 27). Do not jump ahead to Milestone 8 or beyond without the user's explicit go-ahead.
